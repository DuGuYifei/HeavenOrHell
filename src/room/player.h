...
public:
    ...
    // Constructors
    Player()
        : conv_id(-1), position{}, hp(100.0f), maxHp(100.0f), character_type(message::CharacterType::SOUL_DOG), is_ready(false), is_start_rec_game_msg(false) {}

    explicit Player(int p_conv_id)
        : conv_id(p_conv_id), position{}, hp(100.0f), maxHp(100.0f), character_type(message::CharacterType::SOUL_DOG), is_ready(false), is_start_rec_game_msg(false) {}

    Player(int p_conv_id, Position p_pos, float p_hp, float p_maxHp, message::CharacterType p_char_type, bool p_is_ready)
        : conv_id(p_conv_id), position(p_pos), hp(p_hp), maxHp(p_maxHp), character_type(p_char_type), is_ready(p_is_ready), is_start_rec_game_msg(false) {}
