...
KcpSession::KcpSession(const uint32_t _conv, const sockaddr_in &addr, const int udpFd, const int roomId, const int playerId)
    : conv(_conv), peerAddr(addr), roomId(roomId), playerId(playerId)
{
...
void KcpSession::recvAll() const {
    ...
    Room *room = manager->getRoom(roomId).get();
    while (true)
    {
